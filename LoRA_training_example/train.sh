...
pretrained_model="./model.ckpt" # base model path | 底模路径
is_v2_model=0                             # SD2.0 model | SD2.0模型 2.0模型下 clip_skip 默认无效
parameterization=0                        # parameterization | 参数化 本参数需要和 V2 参数同步使用 实验性功能
train_data_dir="./train/image"              # train dataset path | 训练数据集路径
...
resolution="640,960"  # image resolution w,h. 图片分辨率，宽,高。支持非正方形，但必须是 64 倍数。
...
unet_lr="1" # U-Net learning rate | U-Net 学习率
text_encoder_lr="1" # Text Encoder learning rate | 文本编码器 学习率
...
# 优化器设置
optimizer_type="prodigy" # Optimizer type | 优化器类型 默认为 AdamW8bit，可选：AdamW AdamW8bit Lion Lion8bit SGDNesterov SGDNesterov8bit DAdaptation AdaFactor prodigy
